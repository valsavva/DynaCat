...
Global $cDecimalplaces, $PointsEnemy, $PointsFood, $PointIds, $minPointsFood, $minPointsEnemy

Global $LayerTagOpen, $GroupTagOpen, $TeleportChain = False
Const $MaxIdCounter = 9
Dim $IdCounter[$MaxIdCounter+1]
Dim $IdCounterNames[$MaxIdCounter+1]
...
$IdCounterNames[8] = "Oil Barrel"
$IdCounterNames[9] = "dealy sec"
...
Func InitParams()
	...
	$PointsFood = 0
	$PointIds = ''
	$minPointsFood = 7777777
	$minPointsEnemy = 7777777
...
Func GetAttr($str, $attr)
	...
	if $attr = "@imageParams" then
		$ret = ''
		
		$a = GetAttr($str, "@rotation")
		if $a <> 0 Then $ret = $ret & 'Rotation="' & $a & '" '
			
		$a = GetAttr($str, "@stretch")
		if $a <> 'false' Then $ret = $ret & 'Stretch="' & $a & '" '
			
		$a = GetAttr($str, "@opacity")
		if $a <> 1 Then $ret = $ret & 'Opacity="' & $a & '" '
		
		$a = GetAttr($str, "@flip")
		if $a <> '' Then $ret = $ret & 'FlipEffects="' & $a & '" '
	...
	if StringInStr($attr, "@counter") <> 0 then  
		...
		if $counterNumber = 8 Then $IdCounter[9] = 0 ; reset for oil image delay animation
		;if ($counterNumber = 9) and (($IdCounter[9] = 1) or ($IdCounter[9] = 2) or ($IdCounter[9] = 3)) then Return $IdCounter[9]/5 ; first, second and third oil spots will appear very fast
	...
	if $attr = "@fileIndex" Then 
		$ret = ''
		$zFn = GetFileName(GetFullPath($str))
		$i = 1
		While (StringIsDigit(StringRight($zFn,$i)) = 1) and (StringLen($zFn) > $i)
			$ret = StringRight($zFn,$i)
			$i = $i + 1
		WEnd
		...
		if (StringIsDigit($ret) <> 1) Then Halt("###020### Have no file index in file : " & GetFileName(GetFullPath($str)))
		Return $ret
	EndIf

	if $attr = "@flip" Then 
		$ret = GetSubAttr($str, "inkscape:label", "Flip")
		if $ret = "Hor" Then Return 'Horizontally'
		if $ret = "Vert" Then Return 'Vertically'
		Return ''
	EndIf
	...
	if $attr = "@fileNameToStrDir" Then
		If StringInStr(GetFileName(GetFullPath($str)), 'left') > 0 Then Return 'Left'
		If StringInStr(GetFileName(GetFullPath($str)), 'right') > 0 Then Return 'Right'
		If StringInStr(GetFileName(GetFullPath($str)), 'up') > 0 Then Return 'Up'
		If StringInStr(GetFileName(GetFullPath($str)), 'down') > 0 Then Return 'Down'
		Return '!!!ERROR!!!'
	EndIf
...
Func GetValue($str)
	$numArr = StringSplit($str, '>')
	$num = $numArr[0] - mod($numArr[0], 2)
	Return StringMid($str, StringInStr($str, ">", 0, $num/2) + 1, StringInStr($str, "<", 0, $num/2+1) - 1 - StringInStr($str, ">", 0, $num/2))
EndFunc
...
Func CountPoints($str)
Local $ff

	if StringInStr($PointIds, GetAttr($str, 'id')) <> 0 Then Return
	
	$ff = GetFileName(GetFullPath($str))
	Switch $ff
		Case 'fish_4_me'
			$PointsFood = $PointsFood + 70
			if $minPointsFood > 70 Then $minPointsFood = 70
		Case 'fish_3_me'
			$PointsFood = $PointsFood + 80 
			if $minPointsFood > 80 Then $minPointsFood = 80
		Case 'fish_6_me'
			$PointsFood = $PointsFood + 90
			if $minPointsFood > 90 Then $minPointsFood = 90
		Case 'fish_5_me'
			$PointsFood = $PointsFood + 100
			if $minPointsFood > 100 Then $minPointsFood = 100
	EndSwitch
			
			
	if StringInStr($ff, 'fishType01') > 0 Then 
		$PointsFood = $PointsFood + 50 
		if $minPointsFood > 50 Then $minPointsFood = 50
	EndIf
	
	if (StringInStr($ff, 'fishType02') > 0 ) or (StringInStr($ff, 'fishType03') > 0 ) or (StringInStr($ff, 'fishType04') > 0 ) Then 
		$PointsFood = $PointsFood + 70 
		if $minPointsFood > 70 Then $minPointsFood = 70
	EndIf
		
	if StringInStr($ff, 'fishType05') > 0 Then 
		$PointsFood = $PointsFood + 100 
		if $minPointsFood > 100 Then $minPointsFood = 100
	EndIf
		
	if (StringInStr($ff, 'fishType06') > 0 ) or (StringInStr($ff, 'fishType07') > 0 ) Then 
		$PointsFood = $PointsFood + 150 
		if $minPointsFood > 150 Then $minPointsFood = 150
	EndIf
			
			
	if StringInStr($str, "ratType01") > 0 Then 
		$PointsEnemy = $PointsEnemy + 50
		$minPointsEnemy = 50
	EndIf
		
	if StringInStr($str, "ratType02") > 0 Then 
		$PointsEnemy = $PointsEnemy + 50
		$minPointsEnemy = 50
	EndIf
	
	if StringInStr($str, "ratType03") > 0 Then 
		$PointsEnemy = $PointsEnemy + 75
		if $minPointsEnemy > 75 Then $minPointsEnemy = 75
	EndIf
	
	if StringInStr($str, "ratType04") > 0 Then 
		$PointsEnemy = $PointsEnemy + 75
		if $minPointsEnemy > 75 Then $minPointsEnemy = 75
	EndIf
	
	if StringInStr($str, "ratType07") > 0 Then 
		$PointsEnemy = $PointsEnemy + 110
		if $minPointsEnemy > 110 Then $minPointsEnemy = 110
	EndIf
		
	$PointIds = $PointIds & '|' & GetAttr($str, 'id')
EndFunc
...
Local $maskFile = ""
	...
	if $el = "g" Then
		...
		if GetAttr($str, "inkscape:groupmode") = "layer" Then	;layer
			$LayerTagOpen = $LayerTagOpen + 1
			$ln = GetAttr($str, "inkscape:label")
			if StringLeft($ln, 1) = '_' Then 
				$cashContent =  ' CacheContent="true"'
			Else
				$cashContent =  ''
			EndIf
			$op = GetSubAttr($str, "style", "opacity")
			if $op <> '' then $op = ' Opacity="' & $op & '"'
			Return '<Layer Id="' & $ln & '"' & $op & $cashContent &' BackColor="#FFFFFFFF">'
		Else	;group
			$GroupTagOpen = $GroupTagOpen + 1
	...
	;text
	if $el = "text" Then
		$ret = '<Group Id="grp' & GetAttr($str, '@counter5') & '" Class="clsHelpText" ClassParams="'& "@type='text',@txt=" & GetValue($str) & ',@txrIndex=1,@blink=false,@rttn=0" Location="' & GetAttr($str, 'x') & ', ' & GetAttr($str, 'y') & '" TraceId="' & GetAttr($str, 'id') & '"/>'
		Return $ret
	EndIf
...
Local $includes = "", $cls, $xmlExist = False
	...
	if VerifySVG() Then
		...
		if StringInStr($inFileName, 'Lvl_Poezd') > 0 Then
			Write('<Include File="Dashboard_Poezd_Series.xml" />', False)		
		Else
		...
		$stat = '<!-- '
		For $i = 1 to $MaxIdCounter
			$stat = $stat & $IdCounterNames[$i] & '=' & $IdCounter[$i] & '  '
		Next
		$stat = $stat & ' -->' & @CRLF
		
		if $PointsFood > 0 Then
			$stat = $stat & '<!-- -->' & @CRLF
			$stat = $stat & '<!-- Food Points = ' & $PointsFood & ' -->' & @CRLF
			$stat = $stat & '<!-- 1% = ' & $PointsFood*0.01 & ';   45% = ' & $PointsFood*0.45 & ';   90% = ' & $PointsFood*0.90 & ' -->' & @CRLF
			$stat = $stat & '<!-- minPointsFood = ' & $minPointsFood & ' (' & Round($minPointsFood*100/($PointsFood),1) & '%) -->' & @CRLF
		EndIf
		
		if $PointsEnemy > 0 Then
			$stat = $stat & '<!-- -->' & @CRLF
			$stat = $stat & '<!-- Enemy Points = ' & $PointsEnemy & ' -->' & @CRLF
			$stat = $stat & '<!-- 1% = ' & $PointsEnemy*0.01 & ';   45% = ' & $PointsEnemy*0.45 & ';   90% = ' & $PointsEnemy*0.90 & ' -->' & @CRLF
		...
		if ($PointsEnemy > 0) and ($PointsFood > 0) Then
			...
			$stat = $stat & '<!-- minPointsFood = ' & $minPointsFood & ' (' & Round($minPointsFood*100/($PointsFood+$PointsEnemy),1) & '%) -->' & @CRLF
			$stat = $stat & '<!-- minPointsEnemy = ' & $minPointsEnemy & ' (' & Round($minPointsEnemy*100/($PointsFood+$PointsEnemy),1) & '%) -->' & @CRLF
		EndIf
