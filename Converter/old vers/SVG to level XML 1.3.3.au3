...
Func Convert()
	...
	if VerifySVG() Then
		...
		Write('<!-- program version: '& $ProgVers &' -->', False)
		Write('<!-- date: ' & @MDAY & '/' & @MON & ' ' & @YEAR & '   ' & @HOUR & ':' & @MIN &' -->', False)
		Write('<!-- source: '& $inFileName &' -->', False)
		Write('', False)
		Write('<Level Name="' & GetFileName($inFileName) & '" Bounds="0, 0, 480, 320">', False)
		Write('', False)
		Write('<LevelSettings Dificulty="-1" BombCount="7777777" HeroHealth="5" ExplosionClass="clsExplosion" />', False)
		Write('<Include File="styleClasses.xml" />', False)
