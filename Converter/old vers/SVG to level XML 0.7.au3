...
Local $path = "", $name = ""
	...
	if ($quickBoot = "True") and ($lastFile <> "") then 
		$inFileName = $lastFile
	Else
		if $CmdLine[0] <> 0 Then $inFileName = $CmdLine[1]
		
		if (not FileExists($inFileName)) or (GetExt($inFileName) <> "svg") Then 
			if GetFileName($lastFile) <> "" Then 
				$name = GetFileName($lastFile) & "." & GetExt($lastFile)
			Else
				$name = ""
			EndIf
			$inFileName = FileOpenDialog("Select input file", GetPath($lastFile), "SVG Files (*.svg)", 1, $name)
	...
	$outFileName = GetFileName($inFileName) & ".xml"
	...
	if MsgBox(289, "Make Arc", 'Make arc?') = 1 Then
		if MsgBox(33, "Dest", 'in Dropbox?') = 1 Then 
			$path = IniRead($iniFile, "path", "DropboxFolder", @DesktopDir)
		Else
			$path = ""
		EndIf
		ClipPut(GetFileName($inFileName))
		$ArcRoot = FileSelectFolder("Select folder for resources root", $path, 1)
	Else
		$ArcRoot = ""
	EndIf
...
if MsgBox(33, "Execute!", 'EXECUTE?') = 1 Then
