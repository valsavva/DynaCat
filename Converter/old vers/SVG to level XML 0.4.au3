...
Global $ElementsDict = ""
Global $FirstLayer = True, $quickBoot = "False"
...
Func CollectAndWriteResources($TakeIniFile)
	...
	Else
		$ResourcesRootFolder = FileSelectFolder("Select folder for resources root", @ScriptDir)
	EndIf
	if $ResourcesRootFolder = "" Then Exit
	Write('<Resources RootFolder="' & $ResourcesRootFolder & '">', False)
	$textures = ""
	Do
		$str = ReadStr()
	...
	for $i = 1 to $texturesArr[0]
		if $texturesArr[$i] <> "" Then
			...
			Write('<Texture Id="' & $filename & '" Source="' & $filename & '" />', False)
			$ContentFolder = IniRead($iniFile, "path", "ContentFolder", "")
			if $ContentFolder = "" then ProgClose()
			if FileExists($ContentFolder & "\" & $ResourcesRootFolder & "\" & $filename & ".png") Then
				if (FileGetSize($fullpath) <> FileGetSize($ContentFolder & "\" & $ResourcesRootFolder & "\" & $filename & ".png")) or (FileGetTime($fullpath, 0, 1) <> FileGetTime($ContentFolder & "\" & $ResourcesRootFolder & "\" & $filename & ".png", 0, 1))  Then
					$randomName = Random(10000, 99999, 1) &  Chr(Random(Asc("A"), Asc("Z"), 1)) & Chr(Random(Asc("A"), Asc("Z"), 1)) & Chr(Random(Asc("A"), Asc("Z"), 1))
					FileCopy($ContentFolder & "\" & $ResourcesRootFolder & "\" & $filename & ".png", $ContentFolder & "\" & $ResourcesRootFolder & "\" & $filename & ".bak~" & $randomName & ".png")
					$filesNotification = $filesNotification & "BACKUP: " & $ContentFolder & "\" & $ResourcesRootFolder & "\" & $filename & ".bak~" & $randomName & ".png" & @CRLF
					FileCopy($fullpath, $ContentFolder & "\" & $ResourcesRootFolder & "\" & $filename & ".png", 9)
					$filesNotification = $filesNotification & "Copied: " & $ContentFolder & "\" & $ResourcesRootFolder & "\" & $filename & ".png" & @CRLF
					FileDelete($ContentFolder & "\" & $ResourcesRootFolder & "\" & $filename & ".xnb")
					$filesNotification = $filesNotification & "DELETED: " & $ContentFolder & "\" & $ResourcesRootFolder & "\" & $filename & ".xnb"
				EndIf
			Else
				FileCopy($fullpath, $ContentFolder & "\" & $ResourcesRootFolder & "\" & $filename & ".png", 9)
				$filesNotification = $filesNotification & "Copied: " & $ContentFolder & "\" & $ResourcesRootFolder & "\" & $filename & ".png" & @CRLF
...
$MetadataFolder = IniRead($iniFile, "path", "MetadataFolder", "/\NODIR/\")

if FileExists($MetadataFolder) Then
	if $quickBoot = "False" Then 
		if MsgBox(33, "Copy", 'Copy file ' & @CRLF & $outFileName & @CRLF & 'to ' & $MetadataFolder & "\" & GetFileName($inFileName) & ".xml" & ' ?') = 1 Then
			FileCopy($outFileName, $MetadataFolder & "\" & GetFileName($inFileName) & ".xml", 1)
