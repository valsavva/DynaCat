...
Func GetAttr($str, $attr)
	...
	if $attr = "@fishType" Then 
		$ret = StringReplace(GetFileName(GetFullPath($str)), 'fishType', '')
		if StringLeft($ret, 1) = '0' Then $ret = StringTrimLeft($ret, 1)
		if StringIsDigit($ret) <> 1 Then Halt("###019### Bad File Name : " & GetFileName(GetFullPath($str)))
		Return $ret
	EndIf
