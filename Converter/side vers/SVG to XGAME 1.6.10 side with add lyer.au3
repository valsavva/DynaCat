...
Global $mFile, $mExit
Global $lFilename, $bSelectFile, $bConvert, $bMakeArc, $cbToDropbox, $cbAndExec, $cbAndShow, $bErrorTxt, $bWipeErrorTxt, $bExit, $bOpenSVG, $bEditSvg, $bCoordsSvg2XGame, $bCoordsXGame2Svg, $bAddNewLayer
...
Local $winW = 406, $winH = 86
	...
	$bAddNewLayer = GUICtrlCreateButton("+", 210, 30, 20, 20)
...
Func AddNewLayer()
	GUICtrlSetState($bAddNewLayer, $GUI_DISABLE)
	m_LogAdd('AddNewLayer()')
	
	;check the level have to be converted
	$cinFileName = $Root & "\" & $Metadata & "\" & GetFileName($inFileName) & ".xml"
	if Not FileExists($cinFileName) Then
		Error($inFileName & ' have to be converted')
		GUICtrlSetState($bAddNewLayer, $GUI_ENABLE)
		Return
	EndIf
	
	;make copy of the converted file
	$OldConvertFile = @ScriptDir & '\tempConvertedFileCopy'
	FileCopy($cinFileName, $OldConvertFile, 1)
	
	;convert level with new layers
	Convert()

	
	;collect exists old layers
	$OldConvertFileH = FileOpen($OldConvertFile, 256)
	MsgBox(0, "$OldConvertFile", $OldConvertFile)
	if $OldConvertFileH = -1 then Halt("#019# fail to open file: " & $OldConvertFile)
	$ExistsOldLayers = ''
	While 1
		$str = FileReadLine($OldConvertFileH)
		If @error = -1 Then ExitLoop
		$str = StringStripWS($str,7)
		if (GetElementName($str) = 'Layer') and (GetTagType($str) <> $cTT_End) Then
			if $ExistsOldLayers <> '' then $ExistsOldLayers = $ExistsOldLayers & '|'
			$ExistsOldLayers = $ExistsOldLayers & GetAttr($str, 'Id')
		EndIf
	Wend
	if $ExistsOldLayers = '' then Halt("#020# no Layers in file! =\ " & $OldConvertFile)
	MsgBox(0, "$ExistsOldLayers", $ExistsOldLayers)
	FileClose($OldConvertFileH)
	
	;collect new layers
	$NewFileH = FileOpen($cinFileName, 256)
	MsgBox(0, "$cinFileName", $cinFileName)
	if $NewFileH = -1 then Halt("#021# fail to open file: " & $NewFileH)
	$NewLayers = ''
	While 1
		$str = FileReadLine($NewFileH)
		If @error = -1 Then ExitLoop
		$str = StringStripWS($str,7)
		if (GetElementName($str) = 'Layer') and (GetTagType($str) <> $cTT_End) Then
			if $NewLayers <> '' then $NewLayers = $NewLayers & '|'
			$NewLayers = $NewLayers & GetAttr($str, 'Id')
		EndIf
	Wend
	if $NewLayers = '' then Halt("#022# no Layers in file! =\ " & $cinFileName)
	MsgBox(0, "$NewLayers", $NewLayers)
	FileClose($NewFileH)
	
	;mark new layers
	$ExistsOldLayersArr = StringSplit($ExistsOldLayers, '|')
	$NewLayersArr = StringSplit($NewLayers, '|')
	
	Fro
	
	_ArrayDisplay($ExistsOldLayersArr)
	_ArrayDisplay($NewLayersArr)
	;merge old with new
	;GetNextLayer()
	
	m_LogAdd('AddNewLayer:OK')
	GUICtrlSetState($bAddNewLayer, $GUI_ENABLE)
EndFunc
